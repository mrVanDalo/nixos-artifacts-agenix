...
<6> xref::options-nixos.adoc#_artifacts_config_agenix_flakestoredir[Path reference where encrypted secrets are stored]
<7> xref::options-nixos.adoc#_artifacts_config_agenix_storedir[Where the serializer writes encrypted files]
<8> xref::options-nixos.adoc#_artifacts_config_agenix_publichostkey[Public keys used to encrypt the secrets for this host]
<9> xref::options-nixos.adoc#_artifacts_config_agenix_publicuserkeys[Additional public keys able to decrypt the secrets]
